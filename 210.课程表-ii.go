...
func findOrder(numCourses int, prerequisites [][]int) []int {
	...
	res := []int{}
	notCircle := true
	// seenMap看看是否成环, -1代表本来访问过，1代表访问完成
	seenMap := map[int]int{}
	for i := 0; i < numCourses; i++ {
		res, notCircle = dfs(adJustMap, seenMap, res, i)
		if !notCircle {
			// 如果成环，返回空数组
			return []int{}
		}
...
func dfs(adJustMap map[int][]int, seenMap map[int]int, res []int, src int) ([]int, bool) {
	if seenMap[src] == -1 {
		return res, false
	...
	for _, vChild := range adJustMap[src] {
		res, notCircle = dfs(adJustMap, seenMap, res, vChild)
		if !notCircle {
			return res, notCircle
		}
	}
