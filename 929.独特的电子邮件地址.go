...
func numUniqueEmails(emails []string) int {
	...
	for _, mail := range emails {
		realMail = getReal(mail)
		if !mailMap[realMail] {
			mailMap[realMail] = true
			count++
		}
	}
	return count
...
func getReal(mail string) string {
	sArr := strings.Split(mail, "@")
	// 对第一个元素进行转化
	localArr := []byte{}
	for _, ch := range sArr[0] {
		if ch == '+' {
			break
		} else if ch != '.' {
			localArr = append(localArr, byte(ch))
		}
	}
	return string(localArr) + "@" + sArr[1]
}
