...
func toGoatLatin(S string) string {
	...
	yuanMap := map[byte]int{'a': 1, 'e': 1, 'i': 1, 'o': 1, 'u': 1,
		'A': 1, 'E': 1, 'I': 1, 'O': 1, 'U': 1}
	for i, length := 0, len(strArr); i < length; i++ {
		if yuanMap[strArr[i][0]] == 1 {
			// 如果首字母元音
			strArr[i] += "ma"
		} else {
			byteArr := []byte(strArr[i])
			byteTmp := byteArr[0]
			byteArr = byteArr[1:]
			strArr[i] = string(byteArr) + string(byteTmp) + "ma"
		}
