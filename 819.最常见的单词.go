...
func mostCommonWord(paragraph string, banned []string) string {
	...
	for _, c := range paragraph {
		ch := byte(c)
		if isAlpha(ch) {
			byteArr = append(byteArr, ch)
		} else if len(byteArr) > 0 {
			singleStr = strings.ToLower(string(byteArr))
			if !isBanned(singleStr, banned) {
				strMap[singleStr]++
				if strMap[singleStr] > maxNum {
					maxNum = strMap[singleStr]
					maxStr = singleStr
				}
			}
			byteArr = byteArr[0:0]
...
func isBanned(str string, banned []string) bool {
	for _, str2 := range banned {
		if str == strings.ToLower(str2) {
			return true
		}
	}
	return false
